...
**Both-family review, always.** The integrated diff is reviewed cold by two reviewers in parallel, one per model family (codex and Claude), fresh each round, neither seeing the other's findings. Why: same-family review inherits the generator's blind spots and produces confident false verification, and a pair covers every line with an opposite-family read whoever built it. This is the one step never dropped, whatever the size of the change, because it is what lets a user merge without reading the diff.
