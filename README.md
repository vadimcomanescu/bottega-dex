...
Human gates are one hosted collaborative doc — comment to change anything, comment `SIGNED` to approve — never walls of markdown in chat. The entire run is isolated: branch `bottega/<feature-slug>` in its own worktree, every commit lands there, and the PR is the only path to trunk — the user's merge click is the only act that lands it. After delivery, the spec is rewritten into a closed, durable record pointing at code and evidence.
