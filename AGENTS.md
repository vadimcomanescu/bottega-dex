...
- Before implementation, freeze the neutral boundary facts: base, owner boundary, exact slice file bounds, non-test LOC, and named test interfaces. Keep the spec or verbatim request separate. Standards receives the neutral facts plus trusted frozen-base authority and no behavior. Spec receives the same neutral facts plus the behavioral baseline. In a multi-builder run, every completed slice is reviewed before integration through the trusted sanitized code-review helper at P2, using the selected high-reasoning engine and its exact-model smoke boundary.
