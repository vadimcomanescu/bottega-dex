...
| `skills/spec` | The commission — blindspot pass, interview, panel-drafted contract (`docs/specs/<YYYY-MM-DD>-<feature-slug>.md`), Direction, shot list; the cross-read guards the solo-drafted path; scenario text lives only in `features/`, authored directly — the doc points, never copies; standalone via `/bottega:spec` (the gate is then the finish line). Template in `references/` |
