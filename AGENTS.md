...
| `skills/signoff` | User gate — the collaborative spec doc (storyboards inline, comment loop, `SIGNED` as a comment); the standing header and doc skeleton in `references/`, `assets/` holds the local canvas for at-the-machine sessions |
