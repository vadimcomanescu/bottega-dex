...
### 2. Native Codex already provides the conductor and worker harness
...
- OpenAI's Superpowers plugin keeps implementer and reviewer prompts beside the conductor skill and dispatches fresh native subagents. [Conductor skill](https://github.com/openai/plugins/blob/11c74d6ba24d3a6d48f54a194cd00ef3beea18f9/plugins/superpowers/skills/subagent-driven-development/SKILL.md), [implementer prompt](https://github.com/openai/plugins/blob/11c74d6ba24d3a6d48f54a194cd00ef3beea18f9/plugins/superpowers/skills/subagent-driven-development/implementer-prompt.md)
- OpenAI's Wix plugin passes a worker an instruction-file path and deliberately keeps worker-only instructions out of the conductor's context. [Wix conductor](https://github.com/openai/plugins/blob/11c74d6ba24d3a6d48f54a194cd00ef3beea18f9/plugins/wix/skills/wix-headless/SKILL.md#L52-L65), [Wix build dispatch](https://github.com/openai/plugins/blob/11c74d6ba24d3a6d48f54a194cd00ef3beea18f9/plugins/wix/skills/wix-headless/references/BUILD.md)
...
The counterevidence matters to the design. Cross-family review should use two strong reviewers against the same evidence. It should not expand into a large panel, majority vote, or weak-model fan-out. The conductor should verify each finding against code and tests instead of treating agreement as truth or disagreement as failure.
...
- Neither reviewer sees the other's findings or the conductor's preferred conclusion.
- The conductor checks findings against evidence and owns the final judgment.
...
| --- | --- | --- | --- |
| Exact GPT-5.6 Sol Ultra hard stop and model guard | [`run` lines 8-10](https://github.com/vadimcomanescu/bottega-dex/blob/13734ba300c2b3979ad569aaf174a238d6ee9738/plugins/bottega-dex/skills/run/SKILL.md#L8-L10), [`hooks.json`](https://github.com/vadimcomanescu/bottega-dex/blob/13734ba300c2b3979ad569aaf174a238d6ee9738/plugins/bottega-dex/hooks/hooks.json) | Remove | Root model choice belongs to the host. The plugin can recommend a capable model but cannot reliably enforce the active task's effort. |
...
| Mandatory native plus Claude review | [`run` line 48](https://github.com/vadimcomanescu/bottega-dex/blob/13734ba300c2b3979ad569aaf174a238d6ee9738/plugins/bottega-dex/skills/run/SKILL.md#L48) | Retain | This is Bottega doctrine. Two strong disjoint families review the same frozen integrated diff independently. The conductor verifies findings rather than voting. |
...
| Fixed three-round review protocol | [`run` line 48](https://github.com/vadimcomanescu/bottega-dex/blob/13734ba300c2b3979ad569aaf174a238d6ee9738/plugins/bottega-dex/skills/run/SKILL.md#L48) | Simplify | Fix confirmed findings and re-review the changed range. Escalate when the design is wrong or progress stalls, not because a universal round counter fired. |
...
2. Keep the active Codex task as conductor. It owns scope, decisions, integration, review arbitration, and delivery.
...
5. Recommend GPT-5.6 with a high or Ultra intelligence level for the conductor in documentation. Do not hard-stop if the host selected something else.
...
2. Start both reviews from the same bundle and source snapshot. Use a fresh native Codex reviewer in read-only mode. In parallel, run `claude -p` through the adapter with `--safe-mode`, a strong Claude model and high effort, `--tools "Read,Glob,Grep"`, `--no-session-persistence`, `--output-format json`, and `--json-schema`.
...
5. Validate reviewer identity from the dispatch channel, not only from self-reported JSON. For Codex, retain the native thread and reported model metadata. For Claude, retain the CLI version, requested route, successful process result, and per-model usage metadata from the JSON envelope. Fail closed when the expected family cannot be established.
6. Keep the reviews blind. Neither reviewer receives the other report, the conductor's narrative, or a generated candidate finding list.
7. The conductor confirms or refutes each finding against the frozen code and relevant checks. Agreement increases confidence but is not required. Disagreement is useful evidence, not a reason to ask a third model automatically.
8. After fixes, run focused verification and one fresh Codex delta review. Re-run Claude only when the fix materially changes the reviewed design or cross-family coverage is otherwise lost. Before delivery, rerun the decisive host gate on the final head.
...
2. **Isolate.** For edits, work on one task branch and worktree when isolation is useful or required. Do not create per-slice worktrees.
3. **Build.** Keep reads, commands, small deterministic edits, and integration in the conductor. Delegate only substantial, bounded implementation with explicit ownership and a verifier.
...
6. **Arbitrate, fix, and recheck.** The conductor confirms or refutes findings with evidence. Resolve confirmed findings, run a fresh Codex delta review, rerun Claude when the fix materially changes the reviewed design, and run the repository's decisive full gate on the final head.
