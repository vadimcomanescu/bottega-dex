...
**Briefs.** A brief carries what only you know; the builder reads the repo for everything else. In it: the slice intent, the interface contract, the files the builder owns, whether sibling slices build in parallel (the owned-files list is binding only then; see `skills/implementing`), the paths to the run's command record and baseline, the files and conventions that matter in that part of the code, any relevant lines from `docs/specs/dead-ends.md`, the instruction to follow `skills/implementing`, and the safety rule (Standing rules). Every brief opens with the worker's first concrete action, and every command in it gets a stated timeout. A command that hits its timeout is an anomaly for the report, not a silent retry.
...
**Commits and the baseline.** Commit messages follow a fixed format (`<slice>: RED ...`, `<slice>: ... (green)`, `bottega: integrate <slice>`); Resume depends on it. Before the first dispatch, every run, a mechanic records the host's commands (test, lint, typecheck, build, acceptance) in `.bottega/run/<feature-slug>/commands.md`; on a multi-slice run it also runs the full host suite and writes the tests already failing on trunk to `.bottega/run/<feature-slug>/baseline.json` (test id + one-line reason). Briefs point at both instead of restating them. Parallel slices build in their own worktrees under `.bottega/wt/<feature-slug>/<slice>/`, managed by the mechanic protocol in [references/integrate.md](references/integrate.md); read it before the first parallel wave. A slice merges only after its gates are green, and the full suite runs at every integrate: any failure not in the baseline freezes further merging until you route the fix.
