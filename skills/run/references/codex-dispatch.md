...
- An output contract ending in a fenced JSON block (verdict, files touched, evidence paths, anomalies), so the `-o` message is parsed like every other worker's report instead of hand-read as prose.
