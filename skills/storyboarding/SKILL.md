...
- **Variants, when the brief asks for A/B:** structurally different wireframes — layout, hierarchy, primary affordance — never color swaps.
...
- **Tooling is whatever the seat verifies present** — any headless browser the seat can install or the brief names for captures; any imaging tool on the machine for strips; the compose script archived beside them.
