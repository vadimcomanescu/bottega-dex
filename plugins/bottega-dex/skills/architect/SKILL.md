...
- Before designing a mechanism, find how the problem is already solved: the platform, the framework, an established dependency, or current best practice. The standard solution is the default. A custom mechanism for a solved problem carries the bugs the standard path already fixed and is unfamiliar to every future reader, so building one is a consequential choice needing a reason a reviewer can inspect.
