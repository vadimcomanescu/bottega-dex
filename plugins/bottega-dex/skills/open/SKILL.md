...
Confirm native subagent dispatch can request `gpt-5.6-sol` at high reasoning. Confirm Claude Code is installed and authenticated with `claude --version` and `claude auth status`, then run the bundled adapter's reviewer dry run against [the review schema](../code-review/references/report.schema.json). Confirm `gh auth status` for the repository host and that the run can read its target repository and base branch. Missing model selection, missing authentication, or a failed dry run stops the run before implementation and tells the user why. Complete when both mandatory review routes and pull-request publication are ready.
