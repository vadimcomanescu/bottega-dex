...
- Run work: a code change clears it. A red check the diff can fix, a `CONFLICTING` merge state, and a `BEHIND` one all land here. Route each through `maestro` to `orchestrate`; for `CONFLICTING` and `BEHIND`, orchestration merges the base branch into the run branch and resolves it. Close's own part is the tail: the repaired, re-reviewed, re-verified head is pushed and re-marked reviewed (phase 2), its fresh QA evidence published (phase 3), and the PR body's evidence links updated so nothing points at the superseded head, then its checks and merge state are re-watched.
