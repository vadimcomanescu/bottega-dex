...
Ask one question at a time, starting with the ones that change the architecture. Give each question your recommended answer, explain what happens today, what each answer changes, and what it costs. When a question can be answered from the codebase or a tool, answer it yourself. When I say I do not understand, explain more fully and ask again. During an active Maestro run, use [domain-modeling](../domain-modeling/SKILL.md) in the isolated run worktree as each decision settles: update each resolved glossary term immediately and write each ADR when that skill's qualification bar is met. When discovery runs on its own, do not edit the caller's checkout. Record the exact proposed glossary and ADR changes in the discovery report and require an isolated delivery run before applying them.
