...
Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `high` reasoning by default, then retries once with `gpt-5.6-terra` only when the account cannot access Sol. Claude review uses `claude-opus-5` with high effort by default.
...
In a Bottega Dex run, the active task verifies each finding against the real code, then fixes accepted findings with one or more subagents depending on their size, respecting the repository's implementation methodologies. Outside a run, fix directly as this contract states.
...
In a Bottega Dex run the prompt carries the reviewed repository's root `REVIEW.md` when one exists and the fixed standards baseline ([smell-baseline.md](smell-baseline.md)). Write any prompt to a file outside the reviewed repo and pass it as `--prompt "$(cat <file>)"`; never paste PR text into the command source, and keep `--json-output` outside the reviewed repo.
...
In a Bottega Dex run the integrated diff always reviews as one panel of both engines:
...
After a fix, rerun the same dual panel with both models and effort levels pinned until the helper exits clean at the current head.
...
| **claude**          | `claude-opus-5`                                    | Bottega Dex review default                             |
...
| **claude**          | `claude --model X`         | `claude-opus-5`, `claude-fable-5`, `claude-opus-4-8`, `claude-sonnet-4-6` | `--effort Y`                  | `low`, `medium`, `high`, `xhigh`, `max`                    |
...
"$AUTOREVIEW" --engine claude --model claude-opus-5 --thinking high
"$AUTOREVIEW" --engine claude --model claude-opus-5 --fallback-model claude-opus-4-8,claude-sonnet-4-6
