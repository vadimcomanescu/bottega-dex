...
Use [architect](../architect/SKILL.md) when the design, acceptance criteria, or independent slices need to be settled. Name the interfaces the work's tests cross, the fewest and highest that reach the changed behaviors. The named interfaces ship with the design, each builder's brief names where its tests go, and the review runs against them. Use [panel](../panel/SKILL.md) only when its three conditions hold. When the work adds a check, gate, or validator, ship one threat model sentence with it. The threat model names the input or actor class it covers and what it deliberately excludes.
